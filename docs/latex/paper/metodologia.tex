...
\author[1,2,3]{Manuel Hidalgo-Pérez\corref{cor1}}
\ead{mhidper@upo.es}
\author[4]{Jorge Díaz-Lanchas}
\ead{jorge.diaz@uam.es}
\author[5]{Miguel Otero-Iglesias}
...
city={Sevilla},
country={España}}
\affiliation[2]{organization={Fundación Real Instituto Elcano},
...
To facilitate the understanding of the proposed methodology, we define below several key concepts used throughout this paper:

\textbf{Trade dependence}.
A country’s exposure to disruptions in supply from another, measured as the proportion of total supply that depends directly or indirectly on that trading partner.

\textbf{Direct dependence (DD)}.
The proportion of a country's total supply that depends immediately on imports directly from another country, without intermediaries. It is expressed as the ratio of bilateral imports to total supply (domestic production plus all imports).

\textbf{Indirect dependence (DI)}.
The proportion of a country's total supply that depends on another through trade routes involving one or more intermediary countries. It captures vulnerabilities that cannot be identified through direct bilateral relationship analysis.

\textbf{Total dependence (DT)}.
The sum of direct and indirect dependencies, representing a country’s total exposure to another, considering all possible transmission channels of vulnerabilities.

\textbf{Hidden dependence}.
The phenomenon where a country’s total dependence on another significantly exceeds its direct dependence (when DT/DD >> 1), revealing vulnerabilities not detectable through traditional trade indicators.

\textbf{Trade path}.
A sequence of countries through which a product or component passes in the global value chain. A path $p = (i, k_1, k_2, ...,j)$ represents how country $j$ may indirectly depend on country $i$ through intermediaries $k_1, k_2$, etc.

\textbf{Path strength [F(p)]}.
A measure that quantifies the intensity with which a disruption in the origin country of a trade path would affect the destination country through that specific route. It considers the dilution of the effect as it passes through each intermediary.

\textbf{Shock propagation}.
The process by which a disruption in one country is transmitted to others through the global trade network, affecting economies that may not have direct relationships with the source of the disruption.

\textbf{Concealment ratio}.
The ratio between total dependence and direct dependence (DT/DD) that indicates the magnitude of vulnerabilities not captured by traditional bilateral analyses. Values greater than 2 suggest significant hidden dependencies.

\textbf{Trade relevance threshold ($\theta$)}.
A parameter that determines which trade flows are sufficiently significant to be considered in the analysis of indirect dependencies, typically expressed as a percentage of total global trade.

\textbf{Convergence threshold ($\varepsilon$)}.
A criterion that determines when to stop the iterative calculation of indirect dependencies, establishing that the inclusion of additional paths no longer significantly alters the result.

\textbf{Economic resilience}.
A system's ability to resist, adapt to, and recover from exogenous disruptions while maintaining its essential functionality. Our indicator allows for the quantitative assessment of this capacity in the face of trade disruptions.
...
\footnotesize
\begin{description}[leftmargin=*, itemsep=1pt, parsep=0pt]
...
where $x_{ij}$ represents the value of imports that country $j$ receives directly from $i$, and $S_j = x_{jj} + \sum_{k\neq j} x_{kj}$ is the total supply of country $j$, summing domestic production $x_{jj}$ and all imports.
...
For a trade path $p = (i, k_1, k_2, ..., k_n, j)$ that starts in country $i$, passes through a series of intermediaries $k_1, k_2, ..., k_n$, and ends in country $j$, we define "path strength" as the measure of how a disruption in country $i$ is transmitted to $j$ through this specific path. This transmission considers how dependence "dilutes" as it passes through each intermediary, a critical aspect that differentiates our approach from more simplified models.
