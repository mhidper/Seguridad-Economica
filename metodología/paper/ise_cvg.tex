...
\title{\textbf{Extensión del Índice de Dependencia Comercial (IDC) mediante\\Cadenas de Valor Globales: Una Metodología Integrada\\para el Análisis de Vulnerabilidades Comerciales}}

\author{Manuel Alejandro Hidalgo-Pérez\thanks{Universidad Pablo de Olavide y Real Instituto Elcano. Email: mhidalgo@rielcano.org} \and Jorge Díaz-Lanchas\thanks{Universidad Autónoma de Madrid. Email: jorge.diaz@uam.es}}
...
\begin{abstract}
Este documento presenta una extensión metodológica del Índice de Dependencia Comercial (IDC) mediante la integración de las Cadenas de Valor Globales (CVG). Mientras que el IDC original identifica dependencias comerciales directas e indirectas entre países, la extensión propuesta descompone estas dependencias según su origen sectorial upstream, revelando las fuentes específicas de vulnerabilidad. La metodología combina matrices Input-Output internacionales (WIOD) con el enfoque de redes comerciales del IDC, proporcionando una herramienta analítica más granular para la formulación de políticas de seguridad económica. Los resultados preliminares muestran que las dependencias aparentemente directas en sectores finales ocultan vulnerabilidades sistémicas en insumos críticos, transformando la comprensión tradicional del riesgo comercial.
...
La creciente fragmentación geoeconómica global ha expuesto vulnerabilidades en las cadenas de suministro que los indicadores tradicionales de dependencia comercial no logran capturar adecuadamente. El Índice de Dependencia Comercial (IDC) desarrollado previamente \citep{hidalgo2024critical} representa un avance significativo al identificar dependencias indirectas a través de intermediarios comerciales. Sin embargo, una limitación fundamental persiste: el IDC agregado no revela las \textit{fuentes sectoriales específicas} de estas dependencias.

Consideremos un ejemplo ilustrativo: España puede mostrar una dependencia del 45\% respecto a China en el sector automóviles según el IDC tradicional. Pero esta cifra agregada no distingue si la vulnerabilidad proviene de:
\begin{itemize}
...
\subsection{Limitaciones del IDC Agregado}

El IDC original calcula la dependencia total como:
\begin{equation}
...
\begin{itemize}
    ...
    \item $DT_{ij}^{u_k}$ es la dependencia en el sector upstream $u_k$ (calculada por el IDC original)
\end{itemize}
...
\section{Metodología: Arquitectura Modular IDC+CVG}
...
\subsection{Componente 2: Integrador IDC-CVG}

El segundo componente integra los coeficientes CVG con el IDC existente para producir descomposiciones de dependencia. Las funcionalidades clave incluyen:
...
\begin{itemize}
    \item \textbf{ITP 2019}: 237 países × 170 sectores para el cálculo del IDC con cobertura geográfica completa
...
La metodología CVG+IDC transforma el concepto de diversificación comercial:
...
La extensión del IDC mediante CVG representa un avance metodológico significativo en el análisis de vulnerabilidades comerciales. Al descomponer dependencias agregadas en sus componentes sectoriales específicos, la metodología revela la anatomía oculta del riesgo comercial moderno.
